        ...
        <div className="mx-auto max-w-4xl">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="mb-4 mt-1 text-sm text-slate-500">
                        支持批量拖拽多个 PDF 文件，或上传 ZIP 压缩包。系统将自动解析文件名并智能匹配至对应的组织架构。
                      </p>
                      <button
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
                    ...
                    <p className="mb-6 h-10 text-sm text-slate-500">
                      当底层审查规则库更新后，无需重新解析文档，仅对已提取的 Facts 重新运行规则引擎。
                    </p>
                    <button
                ...
                <div className="flex items-start justify-between rounded-xl border border-danger-100 bg-danger-50 p-6 shadow-sm">
                  <div>
                    ...
                    <p className="mt-1 max-w-xl text-sm text-danger-700/80">
                      清除系统中版本号低于 v2.0 的历史结构化入库数据（NS 表数据、Facts 等）。此操作不可逆，通常在系统大版本升级后执行以释放存储空间。
                    </p>
                  </div>
                  <button
          ...
          {activeTab === "system" && (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <p className="mt-2 text-sm">包含模型参数配置、规则阈值设定等功能</p>
            </div>
          )}
        </div>
      </div>

      {isUploadModalOpen && <BatchUploadModal onClose={() => setIsUploadModalOpen(false)} />}
    </div>
  );
}
