...
import { formatDateTime, getDisplayIssueTotal, getHighRiskCount, getUiTaskStatusMeta, normalizeUiTaskStatus, toUiTask } from "@/lib/uiAdapters";
        ...
        <tbody className="divide-y divide-border">
          {props.loading ? <tr><td colSpan={7} className="p-8 text-center text-slate-500">正在加载任务...</td></tr> : props.jobs.length === 0 ? <tr><td colSpan={7} className="p-8 text-center text-slate-500">{props.normalizedSearchQuery ? "当前筛选条件和搜索关键字下暂无相关报告" : "当前筛选条件下暂无相关报告"}</td></tr> : <>
            ...
            return <tr key={job.job_id} className={cn("transition-colors hover:bg-slate-50", props.selectedTasks.includes(job.job_id) && "bg-primary-50/50 hover:bg-primary-50/80")}>
              ...
              <td className="p-4"><div className="flex flex-wrap gap-2">{(() => { const statusMeta = getUiTaskStatusMeta(job); const toneClass = statusMeta.tone === "green" ? "border-success-200 bg-success-50 text-success-700" : statusMeta.tone === "orange" ? "border-orange-200 bg-orange-50 text-orange-700" : statusMeta.tone === "red" ? "border-danger-200 bg-danger-50 text-danger-700" : "border-warning-200 bg-warning-50 text-warning-700"; return <span title={task.reviewReasons?.join("；") || undefined} className={cn("rounded-full border px-2.5 py-1 text-xs font-medium", toneClass)}>{statusMeta.status === "analyzing" ? "分析中" : statusMeta.label}</span>; })()}{String(job.report_kind ?? "").trim().toLowerCase() === "unknown" ? <span className="rounded-full border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-700">类型待识别</span> : null}{needsIngestReview(job) && Number(job.review_item_count ?? 0) > 0 ? <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700">待复核 {job.review_item_count}</span> : null}</div></td>
              ...
              <td className="relative p-4 text-center">
                ...
                {props.openMenuId === job.job_id ? <div ref={props.menuRef} className="absolute right-8 top-10 z-30 w-48 rounded-lg border border-border bg-white py-1 shadow-lg">
                  <button type="button" data-testid={`job-view-${job.job_id}`} onClick={() => props.routerPush(`/task/${job.job_id}`)} className="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-slate-700 hover:bg-slate-50"><Eye className="h-4 w-4 text-slate-400" />查看复核详情</button>
