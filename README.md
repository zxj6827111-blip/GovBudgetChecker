...
## 当前状态（2026-03-02）
- 已具备生产化基础门禁：鉴权默认开启、持久化上传目录、可恢复任务队列、就绪检查、CI/E2E 门禁、发布与回滚手册；实际发布仍需结合目标环境完成数据库、Worker、对象存储和回滚演练。
- 验证结果以当前分支的 CI 和本地测试记录为准，不以本文档中的历史状态作为上线依据。
...
- PostgreSQL 保存原件哈希、稳定 `storage_key`、版本、结构化表格/事实和审校结果；原始 PDF 不写入数据库二进制字段，须与数据库一并备份 `UPLOAD_DIR`。
