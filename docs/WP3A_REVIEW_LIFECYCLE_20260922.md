...
### 7.2 跨存储一致性（独立评审第二轮 P1 修复）

复核涉及**三种存储**，它们之间没有也**无法有**共同的原子事务：
...
`conn.transaction()` 只管第一行。把文件写入放进事务体内执行，会得到两种说谎的状态：
数据库回滚了但文件已经改了（一条孤儿编辑锁把问题永久锁住），或者数据库提交失败
但审计里已经写了"复核完成"。整轮独立评审还指出，此前代码注释里"槽位状态、编辑锁、
审计与会话写入在同一个事务里提交"的说法是**错的**，已全部改正。

现在的规则只有一条：**事务内只做数据库事实；文件副作用在提交成功之后执行。**

| 副作用 | 时点 | 失败语义 |
| --- | --- | --- |
| `set review_lock`（复核完成） | **事务内**（写完之后事务才提交） | **fail-closed**：写不进去就抛错 → 数据库回滚 → 复核不成功。它是安全约束，没落下就不允许宣称复核完成 |
| `clear review_lock`（失效 / 重开） | 提交**之后** | fail-closed 方向：清锁失败保留锁（用户暂时改不了问题，可以重开复核），不会出现"数据库说已失效、文件却把问题解锁了" |
| success audit（start / complete / reopen / invalidate） | 提交**之后** | best-effort：数据库已提交，审计失败只记运维错误，绝不反向把业务结果说成失败 |

两个细节值得单独说明：

- **补偿。** "锁在事务内写、提交失败"这个反向窗口必须收口：提交失败时按
  `(job_id, review_session_id, analysis_basis_token)` 三键**精确比较**后清除本次写入的锁。
  不能无条件按 `job_id` 清——并发下文件里可能已经是另一个会话的锁，无条件清会把
  后来那次复核的安全约束一起摘掉。最危险的一档是"basis 相同、session 不同"
  （同代分析上重开复核，basis token 不变），用例逐档覆盖了四种组合。
- **审计为什么不做成硬保证。** 真要"审计与业务同生共死"需要数据库 outbox，
  属于另一个层面的改造，本轮不做（评审意见也明确不要引入完整 event bus）。
  残留窗口是"提交成功到审计落盘之间进程崩溃 → 少一条审计"，已在已知风险里列出。

真库用例（6 条）覆盖每条失败路径，并做过变异验证：把编辑锁失败改回"吞异常"、
去掉补偿、让回滚也执行文件副作用、让 compare-and-clear 忽略 session，
分别会打红对应用例。
...
- **成功审计 = 服务层唯一写入口**，且写在 PostgreSQL **提交之后**（`_audit_event`
  构造、`_append_audit_best_effort` 落盘；构造与写入分开正是为了让时点可控）；
- **路由层只写被拒绝的动作**（``result="rejected"``，带 ``error`` 与 blocker 码），
  因为"这次请求被拒绝"是 API 边界上的事实，与"业务状态改变了"不同。

真库用例断言：回滚场景 0 条 success；第一次 complete 恰好 1 条；重复 complete 0 条新增；
并发 complete 也只 1 条。
...
| P1（第二轮）文件锁 / JSONL 审计不属于 PG 事务 | **成立**。已修：事务内只做数据库事实、文件副作用移到提交之后；`set review_lock` 改为 fail-closed；新增按三键精确比较的补偿；审计改为提交后写；注释与文档中"三者原子提交"的错误说法全部改正 |

## 已知风险

0. **跨存储残留窗口**：编辑锁与审计不在数据库事务内。编辑锁的失败方向已经收口
   （写失败 → 复核失败；提交失败 → 精确补偿），剩余窗口是"提交成功到审计落盘之间
   进程崩溃 → 少一条审计事件"。要做到审计与业务同生共死需要数据库 outbox，
   本轮明确不做。
